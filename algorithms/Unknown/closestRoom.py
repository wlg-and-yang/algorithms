...
def closestRoom_ref(self, rooms: List[List[int]], queries: List[List[int]]) -> List[int]:
    # 按房间面积降序排列
    rooms.sort(key=lambda x: x[1], reverse=True)
    results = [-1] * len(queries)
    query_indices = sorted(range(len(queries)), key=lambda x: queries[x][1], reverse=True)
    ...
    for query_index in query_indices:
        ...
        # 将满足面积条件的房间加入可用房间列表，并保持排序
        while room_index < len(rooms) and rooms[room_index][1] >= target_size:
            binary_insert(available_rooms, rooms[room_index][0])
            room_index += 1
        
        # 查找最接近的房间
        closest_room = -1
        left, right = 0, len(available_rooms) - 1
